...
# 加载和预处理数据
dataset_df = pd.read_csv('text_label.csv', sep=',', header=None, encoding="UTF-8")

# 标签编码
lbl = LabelEncoder() # 初始化 LabelEncoder，用于将文本标签转换为数字标签
labels = lbl.fit_transform(dataset_df[1].values[:507]) # 拟合数据并转换标签，得到数字标签

# 获取label编码顺序
CATEGORY_NAME = list(lbl.classes_)
# config中CATEGORY_NAME的配置顺序来源
print(CATEGORY_NAME)

# 提取文本内容
texts = list(dataset_df[0].values[:507])
...
model = BertForSequenceClassification.from_pretrained(
    './bert-base-chinese', num_labels=6 # 判别式模型，分多少类
)
...
if best_model_path:
    ...
    print("Best model saved to bert.pt")
else:
