...
# 加载和预处理数据
dataset_df = pd.read_csv('../dataset/ecommerce_intents_full.csv', sep='\t', header=None)
# 2. 清洗数据
dataset_df = dataset_df.dropna()  # 去除空值
dataset_df = dataset_df.reset_index(drop=True)

# 3. 确保标签在0-6范围内
unique_labels = dataset_df[0].unique()
...
labels = lbl.fit_transform(dataset_df[0].values[:1000])
# 提取前500个文本内容
texts = list(dataset_df[1].values[:1000])
...
with open('../assets/weights/label_encoder.pkl', 'wb') as f:
    pickle.dump(lbl, f)

# 从预训练模型加载分词器和模型
tokenizer = BertTokenizer.from_pretrained('../assets/model/bert-base-chinese')
model = BertForSequenceClassification.from_pretrained(
    '../assets/model/bert-base-chinese', num_labels=7 # 判别式模型，分多少类
)
...
training_args = TrainingArguments(
    output_dir='../assets/weights/', # 训练输出目录，用于保存模型和状态
    num_train_epochs=4,                  # 训练的总轮数
    per_device_train_batch_size=100,      # 训练时每个设备（GPU/CPU）的批次大小
    per_device_eval_batch_size=100,       # 评估时每个设备的批次大小
