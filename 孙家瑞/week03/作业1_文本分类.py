...
class LSTMClassifier(nn.Module):
    """
    LSTM：三重门控（遗忘门、输入门、输出门）
    细胞状态（cell state） 提供一条“高速公路”，梯度可近乎无损地流过
...
class RNNClassifier(nn.Module):
    """
    其实这个东西因为
    早期时间步（如第 1 个字）几乎收不到梯度信号 → 模型无法学习长距离依赖
    只能通过一些手段来让他勉强可以工作
    但是还是梯度基本上不会下降
    """
    ...
    def forward(self, x):
        embedded = self.embedding(x)  # (batch_size, seq_len) -> (batch_size, seq_len, embedding_dim)
        output, hidden = self.rnn(embedded)  # hidden: (num_layers=1, batch_size, hidden_dim)
        # 取最后一层的 hidden state (因为 num_layers=1, 所以 hidden[0] 就是最终状态)
...
class GRUClassifier(nn.Module):
    """
    计算更快，梯度下降效果更显著，因为只有两个门在控制，更新门+重置门，可以保证梯度不会衰减
    """
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim):
    ...
    def forward(self, x):
        embedded = self.embedding(x)  # (batch_size, seq_len) -> (batch_size, seq_len, embedding_dim)
        output, hidden = self.gru(embedded)  # hidden: (num_layers=1, batch_size, hidden_dim)
        # 取最后一层的 hidden state
        out = self.fc(hidden.squeeze(0))  # (batch_size, output_dim)
        return out
