...
系统检索时，通过计算用户提问向量 $\mathbf{u}$ 与库中候选向量 $\mathbf{v}$ 的余弦相似度 (Cosine Similarity) 来评估相关性：$$\text{similarity} = \cos(\theta) = \frac{\mathbf{u} \cdot \mathbf{v}}{\|\mathbf{u}\| \|\mathbf{v}\|}$$该值越接近 1，表示两个问题的语义越一致。
